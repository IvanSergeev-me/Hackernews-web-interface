...
const ChangeTheme: FunctionComponent<ChangeThemeProps> = () => {
    ...
    const themes: SelectOptions = Object.keys(supportedThemes).map((theme) => {return ({text:theme, value:theme})});
    const handleChangeTheme = (theme:SelectOptionValue) =>{
        switchTheme(theme as Themes)
    }
    return ( 
    <div>
        <DropdownSelect id="change-theme" name="change-theme" options={themes} defaultValue={currentTheme} onChange={handleChangeTheme}/>
    </div> );
}
